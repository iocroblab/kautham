...
namespace libProblem {
  ...
  bool ConsBronchoscopyKin::solve(){
	  if(_inverse) _robConf = solveInverse(_target);
	  else _robConf = solve(_target);
    
    return true;
  }


	RobConf ConsBronchoscopyKin::solveInverse(vector<KthReal> &values){
...
//cout<<"alpha = "<<curr_alpha<<" psi = "<<curr_psi<<endl;
    ...
    RobConf* _CurrentPos = _robot->getCurrentPos();
	  ...
	  RnConf& currRn = _CurrentPos->getRn();
    ...
    mt::Transform T_RzDalpha1;
    mt::Transform T_RzDalpha2;
	...
	x = PosTool[0]-PosSE3[0];
	y = PosTool[1]-PosSE3[1];
	z = PosTool[2]-PosSE3[2];
	*/

	mt::Transform DiffTransf = currTran.inverse()*ToolTransf;
	mt::Point3 PosDiff = DiffTransf.getTranslation();
	...
	//cout<<"PosSE3=("<<PosSE3[0]<<", "<<PosSE3[1]<<", "<<PosSE3[2]<<endl;
	//cout<<"PosTool=("<<PosTool[0]<<", "<<PosTool[1]<<", "<<PosTool[2]<<endl;
	//cout<<"Diff=("<<x<<", "<<y<<", "<<z<<endl<<endl;

    T_RzDalpha1.setRotation(mt::Rotation(0.0,0.0,0.0,1));
    T_RzDalpha1.setTranslation(mt::Vector3(x,y,z));
    T_RzDalpha2.setRotation(mt::Rotation(RzDalpha));
    T_RzDalpha2.setTranslation(mt::Vector3(0,0,0));
	T_RzDalpha =  T_RzDalpha1* T_RzDalpha2* T_RzDalpha1.inverse();
		
    //mt::Transform Tbase;

//changing currTran
	vector<KthReal>& rncoor= currRn.getCoordinates();
