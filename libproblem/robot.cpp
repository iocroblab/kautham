...
namespace libProblem {
  ...
  void Robot::SolveConstrainedKinematics(vector<KthReal> &values){
    ...
    RobConf _robconf = _constrainKin->solve(values);
    Kinematics(_robconf);
  }

  vector<float> Robot::constrainedinterpolate(vector<float> coords, vector<KthReal> &values){
	vector<KthReal> vecTmp;
	control2Parameters(coords,vecTmp);
	vector<KthReal> vecTmpSE3;
	vecTmpSE3.resize(7);
	for (int i=0; i<7; i++){
		vecTmpSE3[i] = vecTmp[i];
	}
	vector<KthReal> currentcoords = deNormalizeSE3(vecTmpSE3);
	_currentConf.setSE3(currentcoords);
			
	RobConf _RobConf = _constrainKin->solve(values);
	_constrainKin->setvalues(0,0);
	_constrainKin->setvalues(0,1);
	_constrainKin->setvalues(0,2);
  ...
  vector<KthReal> Robot::constrainedparameter2Pose(vector<KthReal> &values){
    std::vector<KthReal> coords(6);
    SE3Conf tmp;

    RobConf* _CurrentPos = getCurrentPos();
