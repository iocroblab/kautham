...
namespace Kautham {
    ...
    */
    ...
    std::string Sample::print(bool onlyVal) {
        std::ostringstream s;
        std::string sal="";
        if(onlyVal){
    ...
    //! Modern copy constructor:
    Sample::Sample(const Sample& s) : 
        _dim(s._dim),
        _coords(s._coords),
        _config(s._config),
        _neighset(s._neighset),
        neighsetdistances(s.neighsetdistances),
        _color(s._color),
        _transparency(s._transparency),
        _connectedComponent(s._connectedComponent),
        withinbounds(s.withinbounds)
    {}
    ...
    void Sample::setNeighs(std::vector<unsigned int> neighs){
        _neighset.clear();
    ...
    double Sample::getDistance(Sample* smp, Kautham::SPACETYPE spc){
        ...
        // Each sample has a std::vector<RobConf> who has a SE3 and a Rn configuration
        // inside.
        double dist=0.0;
        ...
        try {
            if ( spc == Kautham::CONFIGSPACE && _config.size() > 0 ) {
                ...
                std::vector<double>& other = smp->getCoords();
    ...
    void Sample::setMappedConf(std::vector<RobConf>& _localConf){
