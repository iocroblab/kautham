...
#include <kautham/planner/omplg/FOSLBKPIECE1.h>
...
ompl::geometric::FOSLBKPIECE1::FOSLBKPIECE1(const base::SpaceInformationPtr &si,
                                            Kautham::omplplanner::omplPlanner *pl,
                                            SynergyTree *st, double exploration,
                                            double initial_lambda, unsigned int update_freq) :
    base::Planner(si,"FOSLBKPIECE1"),
    dStart_(std::bind(&FOSLBKPIECE1::freeMotion,this,std::placeholders::_1),
            std::bind(&FOSLBKPIECE1::distanceFunction,this,std::placeholders::_1,std::placeholders::_2),
            std::bind(&FOSLBKPIECE1::getDefaultNearestNeighbors,this)),
    dGoal_(std::bind(&FOSLBKPIECE1::freeMotion,this,std::placeholders::_1),
           std::bind(&FOSLBKPIECE1::distanceFunction,this,std::placeholders::_1,std::placeholders::_2),
           std::bind(&FOSLBKPIECE1::getDefaultNearestNeighbors,this)),
    ...
    Planner::declareParam<double>("range",this,&FOSLBKPIECE1::setRange,
                                  &FOSLBKPIECE1::getRange,"0.:1.:10000.");
    Planner::declareParam<double>("tree_bias",this,&FOSLBKPIECE1::setTreeBias,
                                  &FOSLBKPIECE1::getTreeBias,"0.:1.");
    Planner::declareParam<double>("border_fraction",this,&FOSLBKPIECE1::setBorderFraction,
                                  &FOSLBKPIECE1::getBorderFraction,"0.:.05:1.");
    Planner::declareParam<double>("failed_expansion_score_factor",this,
                                  &FOSLBKPIECE1::setFailedExpansionCellScoreFactor,
                                  &FOSLBKPIECE1::getFailedExpansionCellScoreFactor);
    Planner::declareParam<double>("min_valid_path_fraction",this,
                                  &FOSLBKPIECE1::setMinValidPathFraction,
                                  &FOSLBKPIECE1::getMinValidPathFraction);
    Planner::declareParam<double>("initial_lambda",this,
                                  &FOSLBKPIECE1::setInitialLambda,
                                  &FOSLBKPIECE1::getInitialLambda);
    Planner::declareParam<double>("efficiency_threshold",this,
                                  &FOSLBKPIECE1::setEfficiencyThreshold,
                                  &FOSLBKPIECE1::getEfficiencyThreshold);
    Planner::declareParam<double>("exploration",this,&FOSLBKPIECE1::setExploration,
                                  &FOSLBKPIECE1::getExploration,"0.:1.");
    Planner::declareParam<double>("update_freq",this,&FOSLBKPIECE1::setUpdateFrequency,
                                  &FOSLBKPIECE1::getUpdateFrequency,"0.:1.e100");
...
void ompl::geometric::FOSLBKPIECE1::omplState2armaVec(const ompl::base::State *s,
                                                      arma::vec &q) const {
    q.resize(vfdim_);
    unsigned int k = 0;
...
void ompl::geometric::FOSLBKPIECE1::armaVec2omplState(const arma::vec &q,
...
ompl::geometric::FOSLBKPIECE1::~FOSLBKPIECE1() {}


void ompl::geometric::FOSLBKPIECE1::setup() {
    Planner::setup();
...
void ompl::geometric::FOSLBKPIECE1::clear() {
    Planner::clear();
...
void ompl::geometric::FOSLBKPIECE1::freeMotion(Motion *motion) {
    if (motion->state) si_->freeState(motion->state);
    delete motion;
}


arma::vec ompl::geometric::FOSLBKPIECE1::getNewDirection(const arma::vec &vrand,
...
void ompl::geometric::FOSLBKPIECE1::updateGain() {
    if (step_ == nth_step_) {
...
ompl::geometric::FOSLBKPIECE1::Motion *ompl::geometric::FOSLBKPIECE1::extendTree
...
void ompl::geometric::FOSLBKPIECE1::updateExplorationEfficiency(Motion *m,
...
ompl::base::PlannerStatus ompl::geometric::FOSLBKPIECE1::solve
(const base::PlannerTerminationCondition &ptc) {
    ...
    while (const base::State *st = pis_.nextStart()) {
        ...
        dStart_.addMotion(motion,xcoord,0./*goal->distanceGoal(st)*/);
    ...
    while (!ptc) {
        ...
        if (dGoal_.getMotionCount() == 0 ||
            ...
            if (st) {
                ...
                motion->root = motion->state;
                projectionEvaluator_->computeCoordinates(motion->state,xcoord);
                dGoal_.addMotion(motion,xcoord,0./*si_->distance(st,startState)*/);
            }
            if (dGoal_.getMotionCount() == 0) {
        ...
        if (motion) {
            projectionEvaluator_->computeCoordinates(motion->state,xcoord);
            disc.addMotion(motion,xcoord,0./*distanceFunction(motion,otherExisting)*/);
            ...
            if (ocell && ocell->data->motions->size() > 0) {
                ...
                if (goal->isStartGoalPairValid(startTree ? otherExisting->root : motion->root,
                    ...
                    si_->copyState(connect->state,otherExisting->state);
                    ...
                    disc.addMotion(connect,xcoord,0./**/);

                    if (isPathValid(disc,connect,tmpState) && isPathValid(otherDisc,otherExisting,tmpState)) {
                        if (startTree) {
...
bool ompl::geometric::FOSLBKPIECE1::isPathValid(Discretization<Motion> &disc, Motion *motion, base::State *temp) {
    ...
    std::pair<base::State*,double> lastValid(temp,0.);
...
void ompl::geometric::FOSLBKPIECE1::removeMotion(Discretization<Motion> &disc, Motion *motion) {
    /* remove from grid */
    Discretization<Motion>::Coord coord;
    projectionEvaluator_->computeCoordinates(motion->state,coord);
    disc.removeMotion(motion,coord);

    Discretization<Motion>::Cell *cell = disc.getGrid().getCell(coord);
    if (cell) {
        cell->data->score *= failedExpansionScoreFactor_;
        cell->data->addCoef *= failedExpansionScoreFactor_;
        cell->data->multCoef *= failedExpansionScoreFactor_;
    }
...
void ompl::geometric::FOSLBKPIECE1::getPlannerData(base::PlannerData &data) const {
