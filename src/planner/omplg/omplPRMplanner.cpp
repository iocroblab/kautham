        ...
        public:
            ...
            void constructRoadmap(const ob::PlannerTerminationCondition &ptc)
            {
                ...
                while (ptc() == false)
                {
                    ...
                    if (grow)
                        //growRoadmap(ob::plannerOrTerminationCondition(ptc, ob::timedPlannerTerminationCondition(2.0 * magic::ROADMAP_BUILD_TIME)), xstates[0]);
                        growRoadmap(ob::plannerOrTerminationCondition(ptc, ob::timedPlannerTerminationCondition(mingrowtime)), xstates[0]);
                    else
                        //expandRoadmap(ob::plannerOrTerminationCondition(ptc, ob::timedPlannerTerminationCondition(magic::ROADMAP_BUILD_TIME)), xstates);
