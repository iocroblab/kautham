...
//! This class define the pure virtual methods of KauthamDEEnvironment and define how the control will be applied on the bodies.
 class TX90Environment: public KauthamDEEnvironment
 {
     public:

    TX90Environment(WorkSpace* ws, KthReal maxspeed, KthReal maxContacts, KthReal minControlsteps,KthReal maxControlsteps, KthReal erp, KthReal cfm,bool isKchain);//!< Constructor define the robot environment(i.e. table environment ) by calling the KauthamDEEnvironment.
     ~TX90Environment(void);
 ...
 */
class TX90StateSpace : public oc::OpenDEStateSpace
{
private:

public:
    TX90StateSpace(const oc::OpenDEEnvironmentPtr &env);//!< Constructor
    ~TX90StateSpace();
 ...
 */
class TX90StateProjectionEvaluator: public ob::ProjectionEvaluator
{

public:
    TX90StateProjectionEvaluator(const ob::StateSpace *space); //!< Constructor
