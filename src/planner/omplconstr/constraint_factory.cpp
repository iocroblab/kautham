...
namespace Kautham {
    ...
    namespace omplconstrplanner{
        ...
        std::shared_ptr<ConstraintFactory::ConstraintBase> ConstraintFactory::createConstraint(const std::string& name, unsigned int ambientDim, unsigned int coDim, double tolerance) {
            auto it = creators_.find(name);
            if (it == creators_.end()) {
                throw std::runtime_error("Constraint type not found: " + name);
            }
            return it->second(ambientDim, coDim, tolerance);
        }

        void ConstraintFactory::registerConstraints() {
            registerConstraint("orientation_ur5",
                [](unsigned int ambientDim, unsigned int coDim, double tolerance) 
                    -> std::shared_ptr<ConstraintFactory::ConstraintBase> {
                        return std::make_shared<OrientationConstraint>(ambientDim, coDim, tolerance);
                }
            );
        }
