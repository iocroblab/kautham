...
namespace Kautham {
    AttachDialog::AttachDialog(QWidget *parent, Qt::WindowFlags f):QDialog(parent, f) {
        wSpace = NULL;
        obs = rob = link = 0;

        setWindowTitle("Choose the object to be attached and the link where will be attached");
        setModal(true);
        setObjectName(QString::fromUtf8("AttachDialog"));
    ...
    void AttachDialog::set(WorkSpace *workSpace) {
        wSpace = workSpace;
        obs = rob = link = 0;
    ...
    bool AttachDialog::run() {
        if (exec()) {
            return(wSpace->attachObstacle2RobotLink(wSpace->getRobot(rob)->getName(),
    ...
    void AttachDialog::fillObstacleBox() {
        obstacleBox->clear();
    ...
    void AttachDialog::fillRobotBox() {
        robotBox->clear();
    ...
    void AttachDialog::changeObstacle(int index) {
    ...
    void AttachDialog::changeRobot(int index) {
    ...
    void AttachDialog::changeLink(int index) {
